...
abstract trait HasIOBinders { this: LazyModule =>
  ...
  InModuleBody {
    ...
    for ((k, v) <- iocellMap) {
      if (v.nonEmpty) {
        val cells = v.groupBy(_.getClass.getSimpleName)

        println(s"  IOBinder for $k generated:")
        for ((t, cs) <- cells) {
          println(s"    ${cs.size} X $t")
          for (c <- cs) {
            println(s"      ${c.iocellName.getOrElse("(unnamed)")} <-> ${c.signalName.getOrElse("(unnamed)")}")
          }
        }
      }
    }
  ...
  (system: HasPeripheryGPIOModuleImp) => {
    val (ports2d, cells2d) = system.gpio.zipWithIndex.map({ case (gpio, i) =>
      gpio.pins.zipWithIndex.map({ case (pin, j) =>
        ...
        val iocell = system.p(IOCellKey).gpio().named(s"gpio_${i}_${j}")
...
class WithSPIIOCells extends OverrideIOBinder({
  (system: HasPeripherySPIFlashModuleImp) => {
    val (ports: Seq[SPIChipIO], cells2d) = system.qspi.zipWithIndex.map({ case (s, i) =>
      ...
      val sckIOs = IOCell.generateFromSignal(s.sck, port.sck, s"${name}_sck", system.p(IOCellKey), IOCell.toAsyncReset)
      val csIOs = IOCell.generateFromSignal(s.cs, port.cs, s"${name}_cs", system.p(IOCellKey), IOCell.toAsyncReset)

      // DQ are bidirectional, so then need special treatment
      val dqIOs = s.dq.zip(port.dq).zipWithIndex.map { case ((pin, ana), j) =>
        val iocell = system.p(IOCellKey).gpio().named(s"${name}_dq_${j}")
        iocell.io.o := pin.o
        iocell.io.oe := pin.oe
        iocell.io.ie := true.B
        pin.i := iocell.io.i
