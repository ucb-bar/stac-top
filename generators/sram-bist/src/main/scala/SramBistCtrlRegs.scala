package srambist

object SramBistCtrlRegs extends Enumeration {
  type Type = Value
  val ADDR, DIN, MASK, WE, SRAM_ID, SRAM_SEL, DOUT, DONE,
      BIST_RAND_SEED, BIST_SIG_SEED, BIST_MAX_ROW_ADDR, BIST_MAX_COL_ADDR,
      BIST_INNER_DIM, BIST_ELEMENT_SEQUENCE, BIST_PATTERN_TABLE,
      BIST_MAX_ELEMENT_IDX, BIST_CYCLE_LIMIT, BIST_STOP_ON_FAILURE, BIST_FAIL,
      BIST_FAIL_CYCLE, BIST_EXPECTED, BIST_RECEIVED, BIST_SIGNATURE, EX = Value

  val REGS = List(
    (ADDR, 13),
    (DIN, 128),
    (MASK, 128),
    (WE, 1),
    (SRAM_ID, 5),
    (SRAM_SEL, 1),
    (DOUT, 128),
    (DONE, 1),
    (BIST_RAND_SEED, 271),
    (BIST_SIG_SEED, 128),
    (BIST_MAX_ROW_ADDR, 10),
    (BIST_MAX_COL_ADDR, 3),
    (BIST_INNER_DIM, 1),
    (BIST_ELEMENT_SEQUENCE, 976),
    (BIST_PATTERN_TABLE, 1024),
    (BIST_MAX_ELEMENT_IDX, 6),
    (BIST_CYCLE_LIMIT, 64),
    (BIST_STOP_ON_FAILURE, 1),
    (BIST_FAIL, 1),
    (BIST_FAIL_CYCLE, 64),
    (BIST_EXPECTED, 128),
    (BIST_RECEIVED, 128),
    (BIST_SIGNATURE, 128),
  )

  val REG_WIDTH = REGS.toMap

  val TOTAL_REG_WIDTH = REG_WIDTH.values.sum

  val SCAN_CHAIN_OFFSET =
    REGS.map(_._1).zip(REGS.map(_._2).scanLeft(0)(_ + _).dropRight(1)).toMap

  val SCAN_OUT_OFFSET =
    REGS.map(_._1).zip(REGS.map(_._2).scanRight(0)(_ + _).drop(1)).toMap
  
  val NUM_REGS = REGS.map{ case (k, v) => (k, (v - 1) / 64 + 1) }.toMap

  val REGMAP_OFFSET =
    (REGS.map(_._1) ++ Iterator(EX))
      .zip(
        REGS.map(_._2).scanLeft(0)((acc, n) => acc + ((n - 1) / 64 + 1) * 8)
      )
      .toMap
}
