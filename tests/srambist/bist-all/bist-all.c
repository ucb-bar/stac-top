...
{
  uint128_t result;
  uint128_t default_wmask = {0xffffffffffffffffLL, 0xffffffffffffffffLL};
  for (int id = 0; id < NUM_SRAMS; id++) {
    printf("Testing initial rw for SRAM %d\n", id);
    sram_params_t params = SRAMS[id];
    sram_test_t test = SRAM_TESTS[id];
    uint128_t data_mask = create_mask(id);

    printf("Writing initial data...\n");
    for (int i = 0; i < params.num_words; i++) {
        srambist_write(i, test.data[i], default_wmask, id);
    }
    printf("Reading back initial data...\n");
    for (int i = 0; i < params.num_words; i++) {
        result = srambist_read(i, id);
        uint128_t expected = and128(test.data[i], data_mask);
        result = and128(result, data_mask);

        if (!eq128(result, expected)) {
            printf("(SRAM %d, ADDR %d) Hardware result 0x%llx%llx does not match reference value 0x%llx%llx\n", id, i, result.hi, result.lo, expected.hi, expected.lo);
            return 1;
        }
    }
    printf("Finished testing initial rw for SRAM %d\n", id);
  }

  for (int id = 0; id < NUM_SRAMS; id++) {
    printf("Testing masked rw for SRAM %d\n", id);
    sram_params_t params = SRAMS[id];
    sram_test_t test = SRAM_TESTS[id];
    uint128_t data_mask = create_mask(id);

    printf("Writing with wmask...\n");
    for (int i = 0; i < params.num_words; i++) {
        srambist_write(i, not128(test.data[i]), test.wmask[i], id);
    }
    printf("Reading back new data...\n");
    for (int i = 0; i < params.num_words; i++) {
        result = srambist_read(i, id);
        uint128_t wmask = create_wmask_mask(id, test.wmask[i]);
        uint128_t expected = and128(or128(and128(test.data[i], not128(wmask)), and128(not128(test.data[i]), wmask)), data_mask);
        result = and128(result, data_mask);
    ...
    printf("Writing with inverted wmask...\n");
    for (int i = 0; i < params.num_words; i++) {
        srambist_write(i, not128(test.data[i]), not128(test.wmask[i]), id);
    }
    printf("Reading back new data...\n");
    for (int i = 0; i < params.num_words; i++) {
        result = srambist_read(i, id);
        uint128_t expected = and128(not128(test.data[i]), data_mask);
        result = and128(result, data_mask);
    ...
    printf("Finished testing masked rw for SRAM %d\n", id);
  }
